...
var resizePizzas = function(size) {
  ...
  function determineDx (pizzaEl, size) {
    var oldwidth = pizzaEl.offsetWidth;
  ...
  function changePizzaSizes(size) {
    var pizzaEls = document.querySelectorAll(".randomPizzaContainer");
    var dx = determineDx(pizzaEls[0], size);
    var newwidth = (pizzaEls[0].offsetWidth + dx) + 'px';
    for (var i = 0; i < pizzaEls.length; i++) {
      pizzaEls[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((scrollTopVal / 1250) + (i % 5));
